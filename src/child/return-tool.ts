...
import { validateCapsuleInput } from "../core/capsule.js";
import type { ResultCapsuleInput } from "../core/types.js";

const ReturnParameters = Type.Object({
  outcome: StringEnum(["complete"] as const, { description: "Marks the investigation ready to return." }),
...
export function createScopeReturnTool(onReturn: (capsule: ResultCapsuleInput) => void): ToolDefinition<typeof ReturnParameters> {
  return {
    name: "scope_return",
    label: "Return scope",
    description: "Return the final bounded result to the parent. Call this alone when the investigation is complete.",
    ...
    async execute(_toolCallId, params) {
      const { outcome: _outcome, ...candidate } = params;
