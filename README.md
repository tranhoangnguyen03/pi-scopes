...
The child gets a copy under `/workspace`, with Bash as its only work tool. It can edit that copy, but **your checkout is not changed**. Network is off by default; owners can explicitly enable bridge networking for project-local dependency installs. The [equipped development image and configuration guide](docker/README.md) provides Node/npm, Python/uv, Git, search tools and build utilities. Parent and child receive the detected on-PATH tools and selected resource/network policy; parent extensions are not inherited. Captured command output, the result capsule, and an automatically captured workspace text patch survive cleanup; guest files do not. The child need not back up files or print a diff. Use `scope inspect/read` to retrieve the patch. Nothing is automatically applied or merged.

Patch capture covers added/modified/deleted UTF-8 regular files and executable-bit changes, against the imported source revision. Capture errors fail the scope explicitly; cancellation can leave capture unavailable. Limits: 12 MiB / 10,000 regular files exported, 20 MiB archive transport, 4 MiB retained patch, 200,000 combined old/new lines per changed file. Only portable ASCII paths (letters, digits, `_`, `.`, `/`, `-`) and ustar-representable names are currently supported. Symlinks, special files, `.git`, unsafe paths, and changed binary/non-UTF-8 files are refused, not silently omitted. New files ignored by the original checkout's Git ignore rules are excluded before archive export, including dependency symlinks and payloads; tracked changes are never ignored. The preliminary listing is bounded to 100,000 non-directory paths / 8 MiB. Global ignore files are disabled. Keep temporary work in `/tmp`.
